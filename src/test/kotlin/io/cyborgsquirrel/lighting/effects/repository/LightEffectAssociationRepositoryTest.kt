...
class LightEffectAssociationRepositoryTest(
    ...
    "Create a light effect association entity" {
        ...
        val newAssociationEntityOptional = associationRepository.findByEffect(lightEffectEntity)

        newAssociationEntityOptional.isPresent shouldBe true

        val newAssociationEntity = newAssociationEntityOptional.get()
        newAssociationEntity.effect shouldBe lightEffectEntity
        newAssociationEntity.strip!!.id shouldBe ledStripEntity.id
        newAssociationEntity.uuid shouldBe associationEntity.uuid
        newAssociationEntity.group?.id shouldBe associationEntity.group?.id
    }
})